...
('eoms',source.eoms, case
 						  ...
 						  when source.eoms ='' THEN ''	
       					  when source.eoms is not null THEN source.eoms	end::text ,'eom.notes',target.eom_notes,null),
('cover_test_dcover_test_n', CONCAT(source.cover_test_d,source.cover_test_n),case 
 ...
 when source.cover_test_d is null and source.cover_test_n is not null THEN CONCAT('@D ',source.cover_test_d ) end::text,'eom.covertest.notes',target.eom_covertest_notes,null)
